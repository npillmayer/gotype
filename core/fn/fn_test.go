...
import (
	...
	"github.com/npillmayer/gotype/core/fn"
)

func TestN(t *testing.T) {
	a := make([]int64, 0, 10)
	for n, N := fn.N().First(); n < 10; n = N.Next() {
		t.Logf("n=%d", n)
		a = append(a, n)
	}
	t.Logf("a=%v", a)
...
func TestR(t *testing.T) {
	a := make([]float64, 0, 10)
	for x, R := fn.R().First(); x < 10; x = R.Next() {
		t.Logf("x=%.1f", x)
		a = append(a, x)
	}
	t.Logf("a=%v", a)
...
func TestF(t *testing.T) {
	seq := fn.N().Where(fn.EvenN()).Map(fn.SquareN())
	t.Logf("seq=%v", seq)
	n, N := fn.N().Where(fn.EvenN()).Map(fn.SquareN()).First()
...
func TestIntFilter(t *testing.T) {
	a := make([]int64, 0, 10)
	for n, N := fn.N().Where(fn.EvenN()).First(); n < 20; n = N.Next() {
		//t.Logf("n=%d", n)
		a = append(a, n)
	}
...
func TestIntMapper(t *testing.T) {
	a := make([]int64, 0, 10)
	i := 0
	for n, N := fn.N().Map(fn.SquareN()).First(); i < 10; n = N.Next() {
		i++
		//t.Logf("n=%d", n)
		a = append(a, n)
	}
